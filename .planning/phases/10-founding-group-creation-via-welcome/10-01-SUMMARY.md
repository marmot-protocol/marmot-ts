...
- **Did NOT run `requirements mark-complete` for FOUND-01/02/03 in `REQUIREMENTS.md`.** Their text describes end-to-end developer-facing behavior ("Developer can create a group with initial invitees...", "After founding creation the group is `Stable` immediately...") that is not reachable until plan 10-03 wires `GroupFactory.create()`'s `invitees` option (and, for FOUND-02, until 10-04's integration test proves it against a real invitee). This plan's own `<source_audit>` table already records FOUND-01/03 as jointly covered by 10-01+10-03 and FOUND-02 by 10-01+10-04. Matches the established precedent in this repo's decision log (Phase 07-01: "Attached only PROOF-02 to requirements.mark-complete -- PROOF-04/05/06 and CUT-02 are also declared on 07-06 frontmatter... and stay Pending in REQUIREMENTS.md until that plan wires the validators into send/inbound/join seams"). All three remain `Pending` in `REQUIREMENTS.md`; the SUMMARY frontmatter's `requirements-completed` field still lists them per the template's mechanical-copy instruction (copy every ID from the plan's own `requirements:` frontmatter), which is a different field from `REQUIREMENTS.md`'s traceability checkboxes.
