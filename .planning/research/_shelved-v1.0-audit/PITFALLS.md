...
**Why it happens:**
The MLS AEAD layer will reject a non-authentic ciphertext anyway, so the _security outcome_ is the same: a forged event
never produces a valid MLS message. But the _order_ matters for spec conformance and for a clean dispose path: the
...
_Pitfalls research for: Marmot v2 dark-matter migration — wire interop audit and closure_
