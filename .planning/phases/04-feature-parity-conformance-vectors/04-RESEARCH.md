...
1. **(RESOLVED) Which portable vectors should be smoke versus extended?**
   - **Decision:** normal `pnpm vitest run` must enumerate and execute every `status: "portable"` entry in `manifest.v1.json`; capability-unsupported entries remain explicit passing contract records rather than skipped/absent tests. This is the static portable smoke set, including `cross-route-own-commit-recovery/v1` and `route-equivalence/v1`, because the manifest itself marks them portable. [VERIFIED: locked decision and current MDK manifest]
   - **Decision:** add offline-catchup cases `0..5` at MDK seed `42` to the normal suite. Those six indices all use the `smoke-24` profile and cover each recovery arm exactly once: natural, reverse, reverse+duplicates, incremental-then-full, restart-before-processing, and competing-commit-waves. [VERIFIED: `refs/mdk/crates/cgka-conformance-simulator/src/offline_catchup_family.rs`]
   - **Decision:** put cases `6..23` in the deterministic extended suite: `6..11` cover all six arms at `medium-96`, `12..17` at `large-384`, and `18..23` at `xlarge-1024`. Phase 4 implements and makes this suite runnable; Phase 5 owns executing it across the full runtime matrix. [VERIFIED: offline family index/profile mapping and locked phase boundary]
   - **Decision:** `incident_replay`, `rust_only`, `rust_only_bridge`, and other non-portable manifest statuses are inventoried with their declared status/capability reason but are not promoted into the Phase 4 portable execution set. [VERIFIED: manifest status field and locked “portable” boundary]

2. **(RESOLVED) Where should durable wrapper IDs live when callers omit optional stores?**
   - **Decision:** add a dedicated optional `ingestStateStore: GenericKeyValueStore<Uint8Array>` sibling option and thread it through `MarmotClient` → `GroupsManager`/`GroupFactory` → `MarmotGroup`. Store a versioned per-group terminal-wrapper ledger under a separate key namespace; do not mix heterogeneous metadata into the required `groupStateStore<SerializedClientState>` and do not overload the rewind-tree blob. This follows the existing typed sibling-store pattern used by `rewindStore` and `removedMarkerStore`. [VERIFIED: `src/client/marmot-client.ts`, `groups-manager.ts`, `group-factory.ts`, `group/marmot-group.ts`, `src/utils/key-value.ts`]
   - **Decision:** when omitted, use an in-memory ledger and document the exact degradation: same-process wrapper dedup remains available, restart-safe wrapper dedup and restart conformance capability are unavailable. Ordinary construction remains valid; the scenario subject reports restart/durable-wrapper capability unsupported unless this store is supplied. [VERIFIED: existing optional-store degradation pattern and capability-declaration decision]
   - **Decision:** every restart-oriented Phase 4 conformance subject must supply a durable in-memory test backend instance that survives client reconstruction. Therefore omission cannot silently pass `restart-delivery-faults` or wrapper replay parity. [VERIFIED: locked restart parity requirement]

3. **(RESOLVED) Does ts-mls expose all canonical snapshot fields without a new narrow accessor?**
   - **Answer:** no. `ClientState` exposes `groupContext`, `keySchedule.exporterSecret`, leaves/tree state, and ts-mls publicly exports `mlsExporter`; however, `groupContextEncoder` exists in `ts-mls/src/groupContext.ts` but is not exported from the package index. The unrelated public `exportSecret(publicKey, cs)` is HPKE external-init functionality and must not be used for the conformance exporter commitment. [VERIFIED: `ts-mls/src/clientState.ts`, `keySchedule.ts`, `groupContext.ts`, `index.ts`]
   - **Decision:** add one narrow named ts-mls export for `groupContextEncoder` (or an equivalently narrow `serializeGroupContext` wrapper) and use `encode(groupContextEncoder, state.groupContext)` for the SHA-256 field. Use the already-public `mlsExporter(state.keySchedule.exporterSecret, "marmot", utf8("convergence-conformance-v1"), 32, ciphersuite)` for the exporter commitment. No larger ts-mls accessor or state serialization shortcut is needed. [VERIFIED: `refs/marmot/foundation/conformance.md` and ts-mls APIs]
   - **Decision:** build the remaining projection in marmot-ts test infrastructure from the public `ClientState` tree/context plus engine lifecycle, convergence status, disposition, unresolved-publication, and effect ledgers. Do not hash `clientStateEncoder` output, because it includes implementation/private state outside canonical equality. [VERIFIED: canonical projection exclusions]

4. **(RESOLVED) How should an unstamped legacy own commit surface?**
   - **Decision:** decode the legacy wire record successfully, but never call `framedCommitProposalsWithSender` or synthesize its ordering key. While `canonicalTipEpoch - authenticatedSourceEpoch <= maxRewindCommits`, retain it as non-candidate deferred input with a new explicit engine reason `missing-own-commit-stamp`, projected to the canonical `deferred` disposition and `missing_history` category. [VERIFIED: locked compatibility decision, convergence horizon contract, and current discriminated result pattern]
   - **Decision:** this condition does not by itself mark the whole group `Unrecoverable`; later authenticating history or migration evidence may make progress possible. It also cannot contribute a truncated branch prefix or compete in winner selection. [VERIFIED: candidate-edge all-or-nothing rule]
   - **Decision:** once `canonicalTipEpoch - authenticatedSourceEpoch > maxRewindCommits`, expire it through the existing beyond-horizon path (`skipped`/`beyond-anchor`, canonical stale-history meaning). With `maxRewindCommits: Infinity`, it remains deferred indefinitely unless repaired. [VERIFIED: `refs/marmot/protocol-core/convergence.md` and existing `beyond-anchor` result]
   - **Decision:** add compatibility tests for decode, within-horizon deferral, no candidate/truncated-prefix admission, finite-horizon expiry, and Infinity retention across restart. [VERIFIED: locked migration requirement]
