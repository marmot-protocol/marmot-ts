...
- **Compatibility: hard cut to v2.** Drop v1/MIP wire compatibility. Darkmatter is already breaking
  ...
  *delete* legacy code instead of maintaining dual paths.
...
### Phase 0 — De-risk ts-mls (spike, blocks 3 + 9)
- Determine whether `app_data_dictionary` / `app_data_update` / `app_components` can be carried via
  ts-mls today, need an upstream contribution, or must be implemented in marmot-ts (custom extension +
  custom proposal handling) or via a ts-mls fork.
- Confirm ts-mls can apply a commit to a retained prior `ClientState` (branch replay) for convergence.
- Output: a decision record that unblocks Phase 3 and de-risks Phase 9. No production code required.
...
**Phase 3 — App-component framework** (`app-components/README.md`, `foundation/registries.md`) — *gated by Phase 0*
