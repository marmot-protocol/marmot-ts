...
**Phases:** 1–5 (incl. inserted 03.1 and 04.1)
**Total Plans:** 53
...
- Phase 03.1: Phase 3 Review Closure (inserted after Phase 3 to close 30 round-3 review findings as planned, verified work instead of a fourth self-graded fix pass)
- Phase 04.1: Terminal Group Disbanding (inserted after Phase 4 for new `marmot.group.lifecycle.v1` spec scope surfaced by the 2026-08-06 reference sweep)

**Key Decisions:**

- Close interop-breakers first (proof v2 → inbound trust/wire boundary) before additive parity work
- Port MDK's `OwnCommitConvergenceStamp` to close the CR-08/CR-11 own-commit convergence defect class structurally (moved from Phase 3 to Phase 4)
- Standing per-phase `refs/marmot` + `refs/mdk` upstream check, committed as its own `chore(refs):` commit
- Cap KeyPackage lifetime inside marmot-ts rather than relying on ts-mls `maximumTotalLifetime` (dead code in rc.14)
- QA-02 byte-exact evidence as immutable dossiers bound to one tested source SHA

**Issues Resolved:**

- Proof v1/v2 wire mismatch with MDK
- Same-id forgery censorship in the 445 drain (WR-01) and the unbounded `rejectedEvents` DoS set
- ForkRecovery unable to replay a device's own applied commit (CONV-04 verify-first)
- Removal events lost during load before registry listeners attached (CR-12)
- Own-commit convergence reconstructed at recovery time instead of stamped at confirm time (CR-08/CR-11)

**Issues Deferred:**

- Multi-device (MIP-06 / MDEV-01), push (MIP-05 / PUSH-01), QUIC data plane, app/tooling crates, encrypted-media/Blossom changes (mdk #852), app-message NIP-40 expiry
- Backlog 999.1–999.6 (group image, docs review, shelved pre-catchup audit/closure phases)
- Upstream `refs/mdk` changes landed after Phase 5 (`2dd92b3`) not yet reviewed

**Technical Debt Incurred:**

- `maxRewindCommits: Infinity` remains memory-unbounded until `GroupHistoryTree` pruning lands
- Stale `MIP-NN` citations remain in `src/` outside the Phase-3 citation manifest
- Accepted review carry-forwards recorded in phase `deferred-items.md` files
